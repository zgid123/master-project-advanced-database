...
import {
  ...
  USER_INVALID_PARAMS_CODE,
  USER_INVALID_PARAMS_NAME,
  USER_NOT_FOUND_CODE,
  USER_NOT_FOUND_NAME,
} from '@domain/auth';
...
export class UserError extends HonoError {
  public static cannotCreate(): UserError {
  ...
  public static alreadyExists(attribute: keyof TUser): UserError {
    return new UserError({
      status: 409,
  ...
  public static invalidParams(attibute: keyof TUser): UserError {
    return new UserError({
      status: 422,
