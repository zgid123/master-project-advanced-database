...
export const usersSubscriptions = pgTable(
  ...
  (table) => {
    return [
      uniqueIndex('users_subscriptions_user_id_follower_id_unique').on(
        table.userId,
        table.followerId,
      ),
    ];
...
export type TUsersSubscription = typeof usersSubscriptions.$inferSelect;

export type TNewUsersSubscription = typeof usersSubscriptions.$inferInsert;
