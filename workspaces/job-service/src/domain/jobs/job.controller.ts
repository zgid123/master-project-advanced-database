...
function getAuthenticatedUserId(request: FastifyRequest): string {
  ...
  if (!isObjectIdHex(userId)) {
    throw new HttpError(
      ...
      'JWT subject must be an ObjectId hex string',
    );
  }
      ...
      schema: {
        ...
        querystring: {
          ...
          properties: {
            cursor: {
              type: 'string',
              description: 'Base64url keyset cursor for list mode only',
            },
    ...
    async (request) => {
      ...
      if (query.q) {
        if (query.cursor) {
          throw new HttpError(
            400,
            'SEARCH_CURSOR_UNSUPPORTED',
            'Search results do not support cursor pagination yet',
          );
        }
